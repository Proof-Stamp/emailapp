...
test.describe('usage stats page', () => {
  test('shows aggregate ProofStamp usage and feedback with the stats layout applied', async ({ page }) => {
    ...
    await expect(page.locator('#feedback-yes')).toHaveText('42')
    await expect(page.locator('#feedback-no')).toHaveText('7')
  ...
  test('handles a deployment without the metrics database', async ({ page }) => {
    ...
    await expect(page.locator('#feedback-yes')).toHaveText('—')
    await expect(page.locator('#feedback-no')).toHaveText('—')
